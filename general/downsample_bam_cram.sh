...
if [ ${IN_FILE: -4} == ".bam" ]; then
	...
	${JAVA} -Xmx30G -jar /hgsc_software/picard/picard-tools-2.6.0/picard.jar DownsampleSam I=${IN_FILE} O=${IN_FILE}_downsampled_${REDUCERATE}.bam P=${REDUCERATE} VALIDATION_STRINGENCY=SILENT TMP_DIR=/space1/tmp;

	${JAVA} -Xmx30G -jar /hgsc_software/picard/picard-tools-2.6.0/picard.jar MarkDuplicates TMP_DIR=/space1/tmp AS=TRUE M=/dev/null VALIDATION_STRINGENCY=SILENT I=${IN_FILE}_downsampled_${REDUCERATE}.bam O=${IN_FILE}_downsampled_${REDUCERATE}.marked.bam
...
if [ ${IN_FILE: -5} == ".cram" ]; then
	...
	/hgsc_software/java/jdk1.8.0_74/bin/java -Xmx30G -jar /stornext/snfs5/next-gen/scratch/phuang/software/picard-2.10.7/build/libs/picard.jar MarkDuplicates TMP_DIR=/space1/tmp AS=TRUE M=/dev/null VALIDATION_STRINGENCY=SILENT I=${IN_FILE}_downsampled_${REDUCERATE}.cram O=${IN_FILE}_downsampled_${REDUCERATE}.marked.bam R=${REFERENCE}
