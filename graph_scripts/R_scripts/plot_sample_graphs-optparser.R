...
option_list = list(make_option(c("-p", "--prefix"), type="character", required=TRUE, 
							   help="Prefix of all Input files (usually it is the sample name",  metavar="character"),
				   make_option(c("-g", "--gFile"), type="character", default=NULL, 
							   help="file name that contains OMIM gene list", metavar="character")
				   );
...
if (is.null(opt$prefix)) {
	...
	stop("Please specify the prefix for the input files (Usually it is the sample name).n", call.=FALSE)
}
