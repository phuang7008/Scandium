...
coverage_plot <- function(fName, chrom_id, chrom_len, type) {
  ...
  if (type == 'wgs') {
    tmp_file <- paste(tmp_file, "_wgs_log_coverage.png", sep = "")
  } else {
    tmp_file <- paste(tmp_file, "_capture_log_coverage.png", sep = "")
  }
...
for (id in id_names) {
  if (id == chrom_id_in) {
    coverage_plot(file_in, id, v37[id], type_in)
    gc()
    break
  }
}
