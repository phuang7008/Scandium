...
# the input file should be the cov.fasta file which contains the coverage for every
# individual bases. This will be at each base resolution
#

coverage_plot <- function(fName, chrom_id, chrom_len, type, low, high, zoom_low, zoom_high) {
  ...
  if (type == 'wgs') {
    tmp_file <- paste(tmp_file, "x_zoomin_WGS_cov_fasta.png", sep = "")
  } else {
    tmp_file <- paste(tmp_file, "x_zoomin_Capture_cov_fasta.png", sep = "")
  }

  title_ <- paste(low, "x-", high, "x_Coverage For Chromosome (Zoom-In)", chrom_id, sep="")
    ...
    if (grepl('>', line)) {
	  ...
	  if (start > 0) {
        ...
        if (length(items) > 0) {
		  ...
		  # capture cov.fasta has start position for each target region
		  # WGS cov.fasta, however, has start/end position for each chromosome only
		  #
          if (type == 'wgs') {
  ...
  cov_plot <- xyplot(df_[,1] ~ 1:nrow(df_), type="p", pch=16, col="darkgreen", cex=0.3, fill="darkgreen",
						...
						ylim=range(c(low,high), na.rm = TRUE), xlim=range(c(zoon_low,zoom_high), na.rm = TRUE),
                        scales=list(x=list(relation='same'), y=list(relation='same')), data=df_, main=title_)
...
version <- args[5]
type_in <- args[6]
zoom_low <- as.numeric(args[7])
zoon_high <- as.numeric(args[8])
...
for (id in id_names) {
  if (id == chrom_id_in) {
    coverage_plot(file_in, id, v[id]+10, type_in, low, high, zoom_low, zoom_high)
    gc()
    break
  }
}
