...
coverage_plot <- function(fName, chrom_id, chrom_len, low, high, zoom_low, zoom_high) {
  ...
  tmp_file <- paste(tmp_file, "_zoom_in_chr", sep = "")
  ...
  cov_plot <- xyplot(df_[,1] ~ 1:nrow(df_), type="p", pch=16, col="darkgreen", cex=0.3, fill="darkgreen",
						...
						ylim=range(c(low,high), na.rm = TRUE), xlim=range(c(zoom_low, zoom_high), na.rm=TRUE),
...
version <- args[5]
zoom_low <- as.numeric(args[6])
zoom_high <- as.numeric(args[7])
...
for (id in id_names) {
  if (chrom_id_in == id) {
	coverage_plot(file_in, id, v[id] + 10, low, high, zoom_low, zoom_high)
	gc()
	break;
  }
}
