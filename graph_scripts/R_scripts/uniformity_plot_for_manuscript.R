...
cov_plot <- ggplot(NULL, aes(x=Position, y=Coverage)) + 
		...
		scale_color_manual(labels = c("Good Sample (H72CKCCXX-2); Uniformity Metric: 0.958", "Bad Sample   (HCC72CCXY-6); Uniformity Metric: 0.269"), values=c("green4","orangered3"), name="Legend") +
...
png(filename = "comparison.png", width=15, height=4, units="in", res=200)
plot(cov_plot)
dev.off()
