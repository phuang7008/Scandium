...
void getGeneExonDetails(MYSQL *con, uint32_t start, uint32_t end, char *chrom_id, Gene_Exon_Array *gene_exon_array) {
	char *sql = calloc(350,sizeof(char));
    sprintf(sql, "SELECT gene_symbol, refseq_name, exon_count, exon_id, start, end FROM Gene_Exons WHERE chrom='%s' AND ((start <= %"PRIu32" AND %"PRIu32" <= end) OR (start <= %"PRIu32" AND %"PRIu32" <= end))", chrom_id, start, start, end, end);
	...
	// need to check the return number and dynamically expand the Gene_Exon list
	uint16_t i=0;
	uint16_t total_returned_count = mysql_num_rows(result);
	if (total_returned_count > 0) {
		gene_exon_array->gene_exons = calloc(total_returned_count, sizeof(Gene_Exon));
		gene_exon_array->size_ge = total_returned_count;

		for (i=0; i<total_returned_count; i++) {
			gene_exon_array->gene_exons[i].chrom_id = calloc(strlen(chrom_id)+1, sizeof(char));
			strcpy(gene_exon_array->gene_exons[i].chrom_id, chrom_id);
		}

		i=0;
	    MYSQL_ROW row;            
		while ((row = mysql_fetch_row(result))) {
			gene_exon_array->gene_exons[i].gene_symbol = calloc(strlen(row[0])+1, sizeof(char));
			strcpy(gene_exon_array->gene_exons[i].gene_symbol, row[0]);

			gene_exon_array->gene_exons[i].refseq_name = calloc(strlen(row[1])+1, sizeof(char));
			strcpy(gene_exon_array->gene_exons[i].refseq_name, row[1]);

			gene_exon_array->gene_exons[i].exon_count = (uint16_t) strtol(row[2], NULL, 10);
			gene_exon_array->gene_exons[i].exon_id    = (uint16_t) strtol(row[3], NULL, 10);

			gene_exon_array->gene_exons[i].start = (uint32_t) strtol(row[4], NULL, 10);
			gene_exon_array->gene_exons[i].end   = (uint32_t) strtol(row[5], NULL, 10);
