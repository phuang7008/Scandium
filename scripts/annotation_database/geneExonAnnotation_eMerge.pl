...
eval {
	$dbh->do("DROP TABLE IF EXISTS Gene_RefSeq_Exon_eMerge");
};

# now create table again
$dbh->do(qq{
  CREATE TABLE Gene_RefSeq_Exon_eMerge (
...
while (<IN>) {
	...
	foreach my $region (@info) {
		...
		$sql = "INSERT INTO Gene_RefSeq_Exon_eMerge VALUES (0, '$items[0]', $items[1], $items[2], $exon_id, 1, $refseq_start, $refseq_end, '$gene_symbol', '$refseq_name')";
...
$sql = "select refseq_name, count(exon_id) from Gene_RefSeq_Exon_eMerge group by refseq_name";
$sth=$dbh->prepare($sql) or die "DB query error: $!";
$sth->execute() or die "DB execution error: $!";
...
foreach my $refseq_name (keys %refseq) {
    ...
    $sql = "UPDATE Gene_RefSeq_Exon_eMerge SET exon_count=$refseq{$refseq_name} WHERE refseq_name='$refseq_name'";
    $sth = $dbh->prepare($sql) or die "Query problem $!\n";
    $sth->execute() or die "Execution problem $!\n";
}
