...
coverage_plot <- function(fName, chrom_id, chrom_len, type, low, high) {
  ...
  if (type == 'wgs') {
    tmp_file <- paste(tmp_file, "x_from_WGS_cov_fasta.png", sep = "")
  } else {
    tmp_file <- paste(tmp_file, "x_from_Capture_cov_fasta.png", sep = "")
  }
...
for (id in id_names) {
  if (id == chrom_id_in) {
    coverage_plot(file_in, id, v[id]+10, type_in, low, high)
    gc()
    break
  }
}
