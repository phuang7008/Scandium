...
coverage_plot <- function(fName, chrom_id, chrom_len, type) {
  ...
  title_ <- paste(type, " Coverage For Chromosome ", chrom_id sep="")
  ...
  if (type == 'High' || type == 'HIGH') {
    y_max = c(10000, max_+10)
  }

  #cov_plot <- xyplot(df_[,1] ~ 1:nrow(df_), type="p", pch=16, cex=0.5, auto.key=list(border=TRUE, cex=0.5), par.settings=simpleTheme(pch=16,cex=0.5), 
  #                    scales=list(x=list(relation='same'), y=list(limits=y_max)), data=df_, main=title_)
...
for (id in id_names) {
  if (chrom_id_in == id) {
	coverage_plot(file_in, id, v37[id] + 1, type)
	gc()
	break;
  }
}
