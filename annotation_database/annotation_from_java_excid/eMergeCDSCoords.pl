...
my $db_snp  = $type=~/hg38/i ? "eMerge_SNP38" : "eMerge_SNP37";
...
eval {
	...
	$dbh->do("DROP TABLE IF EXISTS $db_snp");
};
...
while (<IN>) {
	...
	if ($gene_name=~/^NM/) {
		...
		# for snp
		#
		$sql = "INSERT INTO $db_snp VALUES (0, '$items[0]', $items[1]-1, $items[2], '$gene_symbol', '$gene_name')";
		$sth = $dbh->prepare($sql) or die "Query problem $!\n";
		$sth->execute() or die "Execution problem $!\n";
	}
}
