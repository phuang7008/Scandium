...
eval {
	$dbh->do("DROP TABLE IF EXISTS Capture_Regions37");
};

# now create table again
$dbh->do(qq{
CREATE TABLE Capture_Regions37 (
...
while (<IN>) {
	...
	my $sql = "INSERT INTO Capture_Regions37 VALUES (0, '$items[0]', $items[1], $items[2], $ex_starts, $ex_ends, $items[5], $items[6], '$gene', '$Synonymous', '$prev_gene', '$annotations')";
