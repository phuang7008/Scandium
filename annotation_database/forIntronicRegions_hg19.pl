...
eval {
	$dbh->do("DROP TABLE IF EXISTS Intron_Regions37");
};

# now create table again
$dbh->do(qq{
CREATE TABLE Intron_Regions37 (
...
while (<IN>) {
	...
	foreach my $sn (keys %source_names) {
		...
		if ($sn=~/^OTT.*/) {
			$sql = "SELECT symbol, prev_symbol FROM HGNC37 WHERE (ucsc_id IS NOT NULL AND find_in_set('$sn', replace(vega_id, '|', ',')))";
		}
	...
	my $sql = "INSERT INTO Intron_Regions37 VALUES (0, '$items[0]', $items[1], $items[2], '$gene', '$Synonymous', '$prev_gene')";
