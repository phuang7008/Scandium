...
if [[ $# -ne 2 ]]; then
	...
	echo "/stornext/snfs5/next-gen/scratch/phuang/git_repo/annotation_database/setup_refseq_exon_annotation_databases_for_liftover_hg19.sh target_bed_file output_dir"
	exit
fi
...
# for hg19: the VCRome+PKv2 is at: /stornext/snfs5/next-gen/capture_designs/HG19_vcrome2.1_with_PKv2.bed
...
/hgsc_software/BEDTools/latest/bin/bedtools intersect -a $refseq_sorted_bed_file_merged_perfect_matches -b $target_bed_file -wb> $exon_target_intersect_file

# Finally, dump everything into MySQL database named: Gene_RefSeq_Exon
#
echo "dump all exons into MySQL database Gene_RefSeq_Exon37 ==> geneExonAnnotation.pl $exon_target_intersect_file"
/hgsc_software/perl/perl-5.18.2/bin/perl /stornext/snfs5/next-gen/scratch/phuang/git_repo/annotation_database/geneExonAnnotation_liftover.pl "$exon_target_intersect_file" "hg19"
