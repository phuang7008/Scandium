...
# This script is used to setup the gene/exon annotation database for sequencing analysis for HG19(HG37)
# First, we need to make sure that user has entered all the required parameters (or arguments)
#
if [[ $# -ne 2 ]]; then
	echo "Illegal Number of Parameters"
	echo "/stornext/snfs5/next-gen/scratch/phuang/git_repo/annotation_database/setup_exon_annotation_databases_hg19.sh miRNA_file output_directory"
	exit
fi
...
cat $HGNC | cut -f 1,2,9,11,21,22,24,25,33 | sed s/HGNC:// | tr -d '"' | sed s/\|/,/g > $HGNC_simplified
printf "dump $HGNC_simplified info into DB using processHGNCtoDB_hg19.pl \n"
#/stornext/snfs5/next-gen/scratch/phuang/git_repo/annotation_database/processHGNCtoDB_hg19.pl "$HGNC_simplified"
...
# for vegaGtp.txt.bed file generated
#rm "vegaGtp.txt.bed";
#awk -F "\t" '{print $1"\t"$2}' vegaGtp.txt > VEGA-hgnc_names ; 
#rm "vegaGtp.txt";
...
# Finally, dump everything into MySQL database named: Exon_Regions37
#
printf "dump all partitioned exons into MySQL database Exon_Regions37 ==> exonAnnotations_hg19.pl $exons_partitioned \n"
/hgsc_software/perl/perl-5.18.2/bin/perl /stornext/snfs5/next-gen/scratch/phuang/git_repo/annotation_database/exonAnnotations_hg19.pl "$exons_partitioned"
